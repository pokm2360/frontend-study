...
function ReservationRefactoring() {
  ...
  const handleInputChange = (e) => {
    ...
    // 불변성을 지켜주어야 리액트 컴포넌트에서 상태가 업데이트 됐음을 감지할 수 있고 이에 따라
    // 재렌더링이 발생함
    // 방법1: 차근차근 방법
    const copyInputs = {
      ...inputs
    ...
    setInputs(copyInputs); // set함수에 새로운 객체 할당

    // 방법2: 코드 단축 버전
    setInputs({
      ...inputs,
      [name]: inputValue // es6차 10장 오브젝트 리터럴
    });

    // 결론: 리액트에서 객체를 업데이트 할 때에는 기존 객체를 직접 수정하면 안되고,
    // 새로운 객체(기존 객체 복사본)를 만들고 그 객체에 변화를 주고 마지막으로 set함수에 넣어줘야함
