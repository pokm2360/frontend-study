...
function ListKey() {
  ...
  // 키 값이 없으면 추가된 아이템 뿐만 아니라 전체가 다 DOM에 업데이트됨(비효율적)
  // 각 아이템에 키를 추가하지 않았기 때문에 리액트는 어떤 항목이 업데이트 됐는지 알지 못함
  // 100개, 1000개 항목이 많아질수록 성능 저하

  return (
    <>
      <input type="text" value={value} onChange={(e)=>{  // 이벤트 핸들러는 보통 e(event)
      ...
      <ul>
        {list.map((item, index)=> {
          ...
          return <li key={index}>{item}</li>; // index를 키 값으로 사용하면 고유하지 않고 바뀔 수 있음
        })}
      </ul>
    </>
  );
